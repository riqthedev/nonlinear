-- Shipline Week 1: cohort pulse, updates, demos, votes (run once on empty DB)
...
create index updates_created_idx on public.updates (project_id, created_at desc);
...
using (
  ...
  and (
    ...
    or exists (
      select 1 from public.cohort_members cm
      where cm.cohort_id = projects.cohort_id and cm.user_id = auth.uid() and cm.role = 'organizer'
    )
  )
)
with check (public.is_cohort_member(projects.cohort_id, auth.uid()));
...
using (
  exists (
    select 1 from public.projects p
    where p.id = updates.project_id and public.is_cohort_member(p.cohort_id, auth.uid())
  )
);
...
create policy demo_events_select on public.demo_events
for select to authenticated
using (public.is_cohort_member(demo_events.cohort_id, auth.uid()));

create policy demo_submissions_select on public.demo_submissions
for select to authenticated
using (
  exists (
    select 1 from public.demo_events de
    where de.id = demo_submissions.demo_event_id
      and public.is_cohort_member(de.cohort_id, auth.uid())
  )
);

create policy demo_submissions_write on public.demo_submissions
for insert to authenticated
with check (
  exists (
    select 1 from public.demo_events de
...
create policy demo_submissions_update on public.demo_submissions
for update to authenticated
using (
  exists (
    select 1 from public.demo_events de
...
using (
  exists (
    select 1 from public.demo_events de
    where de.id = votes.demo_event_id
      and public.is_cohort_member(de.cohort_id, auth.uid())
  )
);
...
with check (
  actor_id = auth.uid()
  and public.is_cohort_member(feed_events.cohort_id, auth.uid())
);
